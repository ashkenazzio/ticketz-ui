...
import { useData, useMyTickets } from '../context/DataContext';
...
const exploreActions = [
    {
        ...
        icon: Calendar,
        path: '/app/search?tab=events',
    },
    {
        title: 'Browse Communities',
        desc: 'Join communities that match your vibe',
        icon: Sparkles,
        path: '/app/search?tab=communities',
    },
];

export default function AppHub() {
    const { db, currentUser } = useData();
    const { upcoming } = useMyTickets();

    // Get communities from database
    const communities = db.communities.getAll();

    // Get upcoming tickets with event info
    const upcomingTickets = upcoming.map(ticket => {
                    ...
                    <section className="px-4 mb-10">
                        <div className="max-w-5xl mx-auto">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {upcomingTickets.slice(0, 2).map(({ ticket, event, tier }) => (
                                    <Link
                                        key={ticket.id}
                                        to={`/event/${event!.id}`}
                                    ...
                                    >
                                        ...
                                        <div className="flex gap-4 p-4">
                                            <div className="w-20 h-20 overflow-hidden flex-shrink-0">
                                                <img
                                                    src={event!.image || 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?q=80&w=400&auto=format&fit=crop'}
                                                    alt={event!.title}
                    ...
                    <div className="max-w-5xl mx-auto">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            {communities.map((community) => {
                                const primaryCategory = getCategoryById(community.primaryCategory);
                                const secondaryCategory = community.secondaryCategory ? getCategoryById(community.secondaryCategory) : null;
                                const PrimaryIcon = primaryCategory?.icon;
                                const SecondaryIcon = secondaryCategory?.icon;
                                ...
                                return (
                                    ...
                                    >
                                        <img
                                            src={community.avatar || 'https://images.unsplash.com/photo-1598387993441-a364f854c3e1?q=80&w=400&auto=format&fit=crop'}
                                            alt={community.name}
                                        ...
                                        <div className="absolute bottom-0 left-0 right-0 p-4">
                                            <div className="flex flex-wrap gap-1.5 mb-2">
                                                {primaryCategory && (
                                                    <span className="inline-flex items-center gap-1.5 bg-lime/30 backdrop-blur-sm text-lime text-xs font-semibold px-2 py-1 border border-lime/20">
                                                        {PrimaryIcon && <PrimaryIcon className="w-3 h-3" />}
                                                        {primaryCategory.label}
                                                    </span>
                                                )}
                                                {secondaryCategory && (
                                                    <span className="inline-flex items-center gap-1.5 bg-purple-500/40 backdrop-blur-sm text-purple-200 text-xs font-semibold px-2 py-1 border border-purple-400/30">
