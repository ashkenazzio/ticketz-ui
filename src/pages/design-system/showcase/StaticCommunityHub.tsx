/**
 * Static version of CommunityHub for design system showcase.
...
import {
  ...
  UserPlus, UserMinus, Shield, BarChart3,
  Bell, BellOff, ArrowLeft, CalendarRange, Settings
} from 'lucide-react';
import MessageBoard from '../../../components/MessageBoard';
import { getCategoryById } from '../../../constants/categories';
import { useData } from '../../../context/DataContext';

// Use first community from database
const SAMPLE_COMMUNITY_ID = 'comm-001';
...
export default function StaticCommunityHub() {
  const [isSubscribed, setIsSubscribed] = useState(true);
  const [notifications, setNotifications] = useState(true);
  const [showAdminMenu, setShowAdminMenu] = useState(true);
  const { db, currentUser } = useData();

  // Get community data from database using hardcoded ID
  const community = db.communities.getById(SAMPLE_COMMUNITY_ID);
  const events = community ? db.events.getByCommunity(community.id) : [];
  const memberships = community ? db.memberships.getByCommunity(community.id) : [];
  const memberCount = memberships.length;

  // Get membership info for current user (mock admin for showcase)
  const isUserAdmin = true; // Show admin panel in showcase
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-6">Sample community data not available.</p>
        </div>
      </div>
    );
  }

  // Get category objects for this community
  ...
  // Get preview members
  const previewMembers = memberships.slice(0, 4).map(m => {
      ...
      <div className="max-w-7xl mx-auto px-6 mt-8">
        ...
        {events.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pb-8">
            {events.filter(e => new Date(e.startTime) > new Date()).slice(0, 3).map((event) => {
