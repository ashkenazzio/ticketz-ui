/**
 * Static version of TicketDetail for design system showcase.
...
export default function StaticTicketDetail() {
  const { db, currentUser } = useData();

  // Get first valid ticket for current user from database
  const ticketData = useMemo(() => {
    if (!currentUser) return null;
    const tickets = db.tickets.getByUser(currentUser.id);
    ...
    return (
      <div className="min-h-screen bg-dark flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-4">No tickets available in sample data.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-dark text-white">
      ...
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        <div className="bg-surface border border-white/10 rounded-sm overflow-hidden mb-6">
          ...
          {/* View Event Link */}
          <div
            className="block border-t border-white/5 p-4 text-center text-sm text-lime hover:bg-lime/5 transition-colors uppercase tracking-wide font-semibold cursor-pointer"
          >
            View Event Details
          </div>
        </div>
